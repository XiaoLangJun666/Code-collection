...
#Source your main
knitr::opts_chunk$set(echo = TRUE)#display the result of the code chunk 
source("main.R")#source the main.R file
```
...
count_mt<-read_data('data/verse_counts.tsv')
...
nonzero_variance_genes<-filter_zero_var_genes(count_mt)
#display the head of the counts data from the zero variance filter function's output
slice_head(nonzero_variance_genes,n=10)
```
...
meta_tb<-meta_info_from_labels(colnames(nonzero_variance_genes)[-1])
meta_tb
...
library(ggplot2)
dt<-as.data.frame(get_library_size((nonzero_variance_genes))/10^6)
dt
ggplot(dt, aes(x = factor(rownames(dt),levels=rownames(dt)), y =dt$`get_library_size((nonzero_variance_genes))/10^6`,fill=factor(rownames(dt),levels=rownames(dt))))+
...
count_cpm<-normalize_by_cpm(nonzero_variance_genes)
head(count_cpm)
```
...
DESeq_normalized_mat<-deseq_normalize(nonzero_variance_genes)
head(DESeq_normalized_mat)
...
plot_sample_distributions(nonzero_variance_genes[-1],scale_y_axis = TRUE,title='Sample Disstributions(Log10(Raw Counts))')
```
...
```{r}
plot_variance_vs_mean(nonzero_variance_genes[-1],scale_y_axis = TRUE,title='Variance vs. Mean (Log10(Raw Counts))')
...
plot_pca(nonzero_variance_genes[-1],meta_tb,title='Raw Counts PCA')
```
...
plot_sample_distributions(count_cpm[-1],scale_y_axis = TRUE,title='Sample Disstributions (log10(CPM))')
plot_variance_vs_mean(count_cpm[-1],scale_y_axis = TRUE,title='Variance vs Mean (log10(CPM))')
plot_pca(count_cpm[-1],meta_tb,title='CPM PCA')
```
...
plot_sample_distributions(DESeq_normalized_mat[-1],scale_y_axis = TRUE,title='Sample Disstributions (log10(DESeq2 norm counts))')
plot_variance_vs_mean(DESeq_normalized_mat[-1],scale_y_axis = TRUE,title='Variance vs. Mean (DESeq2)')
plot_pca(DESeq_normalized_mat[-1],meta_tb,title='DESeq2 Normalized PCA')
```
