...
for t in dt.keys():
    ...
    dc[t] = np.concatenate((matrix2,matrix5,matrix6), 1)

"""
建立一个新的字典，字典的key是蛋白质的标签，以上的matrix1，2，3，4，5，6分别代表 blo62，pssm，psi-pred，asaquick和onehot，可以按照需要在在最后一句代码中更改所需要的特征组合
"""
...
#     return x
"""
分别建立两个seq的函数，第一个用来提取我们新学习到的特征，第二个用来存放原有的onehot矩阵的特征
"""
...
for dataset in datasets:
    ...
    if ((not os.path.isfile(processed_data_file_train)) or (not os.path.isfile(processed_data_file_valid)) or (not os.path.isfile(processed_data_file_test))):
        ...
        """
        上面的更改首先讲两个不同的提取方法单独运行，然后存储到一个小的矩阵中，在将小的矩阵添加到最终矩阵的末尾形成大的矩阵。
        """
